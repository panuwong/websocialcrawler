...
exports.getPost = function (data, rowx) {
  console.log(" ");
  console.log(" ");
  console.log(" ");
  console.log(" ");
  ...
  async function getPost(data, rowx, proxyData) {
    (async () => {
      try {
        ...
        index++;
        browser.close()
        await getPost(data, index, proxyData)
            ...
            db.collection(index_collection).update(
              ...
              function (err, rs) {
                if (err) {
                  ...
                  // console.log("update voice_facebook Success: NO."+rowx);
                  rowx++;
                  addMongo(campaign_id, campaign_set, search_keyword,index_id, data, rowx)
                }
              });
    ...
    (async () => {
      try {
        ...
        for (var index = rowx; index < data.length; index++) {
          ...
          await page.goto("https://www.facebook.com/search/posts/?q=" + keyword + "&epa=FILTERS&filters=eyJycF9hdXRob3IiOiJ7XCJuYW1lXCI6XCJtZXJnZWRfcHVibGljX3Bvc3RzXCIsXCJhcmdzXCI6XCJcIn0ifQ%3D%3D")
          await page.waitFor(2000);
  ...
  async function scrolledpage(page, campaign_set, campaign_id, key, startCmp) {
    try {
      ...
      while (ii < 50) {
        ...
        // //==========================
        try {
          await page.addScriptTag({ content: `${manageindexpage}` });
          var rs2 = await page.evaluate(({ campaign_id, key }) => {
            return manageindexpage(campaign_id, key);
          }, { campaign_id, key });
