...
class Facebook {
  ...
  //   async function getSinglePostOnlys(data, rowx, proxyData, loginyet) {
  //     (async () => {
  //       try {
  ...
  //         index++;
  //         browser.close()
  //         await getSinglePostOnlys(data, index, proxyData, 1)
  //       } finally {
  //         browser.close()
  ...
  getSinglePostOnlyNotispage(data, rowx) {
    ...
    voiceupdate.proxyData("facebook", function (proxyData) {
      ...
      getSinglePostOnlysNotispage(data, rowx, proxyData);



    });

    async function getSinglePostOnlysNotispage(data, rowx, proxyData) {
      (async () => {
        try {
          ...
          for (var index = rowx; index < data.length; index++) {
            ...
            await page.goto(urlgo,{ waitUntil: 'networkidle2' })
            ...
            try {
              ...
              if (result) {
                ...
                index++;
                browser.close()
                getSinglePostOnlysNotispage(data, index, proxyData)
              }
          ...
          index++;
          browser.close()
          await getSinglePostOnlysNotispage(data, index, proxyData)
        } finally {
          browser.close()
          process.kill(browser.process().pid);
        }
    ...
    async function addpost(page, url, index_id, timetostamp) {
      try {
        console.log('Click SHOW LIKE POST');
        let secone1 = await page.$('div[data-insertion-position="0"]')
        if (!(secone1) || secone1==="undefined" || secone1===null || secone1 === "") {
          secone1 = await page.$('.userContentWrapper')
        }
        const checkemo1 = await secone1.$('._3t54 > a._3emk._401_')
        if ((checkemo1)) {
          await page.click('._3t54 > a._3emk._401_');
        }else{
          await page.click('._1n9l');
        ...
        const rs = await page.evaluate(({url,index_id}) => {
          ...
          return new Promise(function (resolve, reject) {
              ...
              var boxsecone = document.querySelector('div[data-insertion-position="0"]');
              if (typeof boxsecone === "undefined" || !(boxsecone)) {
                boxsecone = document.querySelector('.userContentWrapper');          
              }
              ...
              if ((nameposter)) {
                ...
                nameposter = boxsecone.querySelector('span.fwb > a');
                nameposter = nameposter.innerText;
              }

              var pgname = document.querySelector('._64-f > span')
                  if ((pgname) && pgname!=='undefined' && pgname!==null) {
                    pgname = pgname.innerText
                  }else{
                    pgname = nameposter
                  }
              ...
              // }
                ...
                var comment = document.querySelectorAll('div.UFICommentActorAndBodySpacing')
                if ((comment) && comment!=='undefined' && comment!==null) {
                  var comlength = comment.length;
                  if (comlength==0) {
                    comment = document.querySelectorAll('._72vr');
                    var comlength1 = comment.length;
                    if (comlength1>0) {
                      comlength = comlength1;
                    }
                  }
                }
              ...
              if (!(comlength) || comlength==0) {
                ...
                if (allemo === 0) {
                  ...
                  if ((document.querySelector('div[data-insertion-position="0"]'))) {
                    var sharesecone = document.querySelector('div[data-insertion-position="0"]');
                  }else{
                    var sharesecone = document.querySelector('.userContentWrapper');
                  }
                  var sharepost = sharesecone.querySelector('._ipm._2x0m');
                  if ((sharepost)) {
                    if (sharepost.innerText.includes('Shares')) {
                      var sharecount = sharepost.innerText.replace('Shares', ' ');
                    } else {
                      var sharecount = sharepost.innerText.replace('Share', ' ');
                    }
                  } else {
                    var sharecount = 0;
                  }
              ...
              if (i == 1) {
                ...
                if ((commentpost)) {
                  ...
                  commentpost = document.querySelector('a._3hg-._42ft');
                  if (commentpost.innerText.includes('Comments')) {
                    commentcount = commentpost.innerText.replace('Comments', ' ');
                  } else {
                    commentcount = commentpost.innerText.replace('Comment', ' ');
    ...
    async function addMongo(camid, camset, keyword, index_id, data, timetostamp, rowx) {
      ...
      try {
        ...
        console.log("Addmongopost ERROR EVALUATE NO DATA ANY VARIABLE",err);
        voiceupdate.readIndexChangeR(index_id, "NS : Evaluate ERR Facebook " + err + " index_id: " + index_id);
      }
    }
