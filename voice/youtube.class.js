...
class Youtube {
    ...
    async getCommentThread(data, rows) {
        try {
            ...
            try {
                const rowxx = rows;
                var result = await voiceupdate.updateStatus(data, rows)
                if (result) {
                    run(data, rows)
                    runcom(data, rowxx)
                } else {
                    ...
                    await request({
                        ...
                        try {
                            ...
                            await db.collection("index_repo_campaign").find({ 'url': fullurl }).toArray(async function (err, result) {
                                ...
                                var camset1 = await setCam(result[0].campaign_id, campaign_set)
                        ...
                        async function setCam(camid, campaign_setArr) {
                            return new Promise(async function(result,reject){
                                try {
                                        ...
                                        await db.collection("voice_youtube").update(
                                            ...
                                            },
                                            {
                                                $set: {
                                                    ...
                                                    "subscribers": "",
                                                    "engagement": parseInt(mains[0].statistics.likeCount),
                                                    // + parseInt(mains[0].statistics.dislikeCount) + parseInt(mains[0].statistics.viewCount),
                ...
                try {
                    ...
                    request({
                        ...
                        if (typeof body !== "undefined") {
                            ...
                            var mains = data.items;
                            console.log("---------------------TOTAL YOUTUBE "+ keyword +" : " + data.pageInfo.totalResults);
