...
var index_collection = "index_repo_campaign_bk"

var index_repo_search = "index_mini_search"

exports.readIndexChangeR = function (index_id, error_status, code_error = 200) {
    (async () => {
...
exports.readIndexChangeY = function (domain, timetostamp) {
    (async () => {
        db.collection('master_index_mini_search').update({ "domain": domain }, {
            $set: { "readindex": 'Y', "readnexttime": timetostamp }
        });// หยิบแล้ว stamp กลับเป็น Y ทันที พร้อมเซทเวลาในการคลอเลออีกครั้ง

        this.d("     update status y in index_id : " + domain);
    })();
}
...
exports.updateIndexRepoSearch = function (data, rowx, callback) {
    
    dataRepoSearch(data, rowx)
    //   callback(d)

    

     function dataRepoSearch(data, rowx) {
        ...
        if ((data[rowx])) {  
             ...
             db.collection(index_repo_search).find({ "url": data[rowx]['url'] }).toArray( function (err, resultcheck) {
                ...
                if (resultcheck.length == 0) {
                    if ((data[rowx])) {
                     db.collection("zprimarykey_index_mini_search").findAndModify(
                        {
                            query: { _id: "indexid" },
                            update: { $inc: { seq: 1 } },
                            new: true
                         ...
                         function (err, result) {
                            ...
                            keepArr['platform'] = 'website'
                            keepArr['platform_typepost'] = ''
                            keepArr['readindex'] = 'Y'
